...
import { isURL, isISBN, isISSN } from 'validator';

const IdentifierFormat = BaseValidator.extend({
  validate(value, options, model) {
    const ark = /^ark:\/[0-9]{5}\/\S+$/;
    const lsid = /^[uU][rR][nN]:[lL][sS][iI][dD]:(.+)/;
    const purl = {
      require_host: true,
      host_whitelist: ['purl.org', 'purl.oclc.org']
    };
    ...
    const types = ['EAN13', 'Handle', 'IGSN', 'ISSN', 'ISTC', 'UPC', 'w3id'];
    ...
    switch (true) {
      ...
      case model.relatedIdentifierType == 'PMID':
        return pmid.test(value) ? true : 'Please enter a valid PMID.';
      ...
      case model.relatedIdentifierType == 'ISSN':
        return isISSN(value) ? true : 'Please enter a valid ISSN.';
      case model.relatedIdentifierType == 'EISSN':
        return isISSN(value) ? true : 'Please enter a valid EISSN.';
      case model.relatedIdentifierType == 'LISSN':
        return isISSN(value) ? true : 'Please enter a valid LISSN.';
      ...
      case types.includes(model.relatedIdentifierType):
        return true;
      default:
        return 'Please enter a valid related identifier type.';
    }
  }
});
