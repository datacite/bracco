...
export function providerFormErrors([model]) {
  ...
  if (
    ...
    isPresent(model.technicalContact.validations.errors)
  ) {
    errorAttributes = errorAttributes.concat(
      'technical contact ' +
        model.technicalContact.validations.errors.mapBy('attribute')
    );
  }
  if (
    model.secondaryTechnicalContact &&
    isPresent(model.secondaryTechnicalContact.validations.errors)
  ) {
    errorAttributes = errorAttributes.concat(
      'secondary technical contact ' +
        model.secondaryTechnicalContact.validations.errors.mapBy('attribute')
    );
  }
  if (
  ...
  if (
    model.secondaryServiceContact &&
    isPresent(model.secondaryServiceContact.validations.errors)
  ) {
    errorAttributes = errorAttributes.concat(
      'secondary service contact ' +
        model.secondaryServiceContact.validations.errors.mapBy('attribute')
    );
  }
  ...
  if (model.memberType !== 'consortium_organization') {
    ...
    if (
      model.secondaryBillingContact &&
      isPresent(model.secondaryBillingContact.validations.errors)
    ) {
      errorAttributes = errorAttributes.concat(
        'secondary billing contact ' +
          model.secondaryBillingContact.validations.errors.mapBy('attribute')
      );
    }
    if (
      model.votingContact &&
