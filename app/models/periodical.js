...
const Validations = buildValidations({
  symbol: [
    ...
    validator('format', {
      ...
      message: 'The Periodical ID must start with the Provider ID, followed by a dot. It can then contain only upper case letters, numbers, and at most one hyphen.'
    }),
    validator('length', {
      min: 5,
      max: 18
  ...
  confirmSymbol: [
    ...
    validator('confirmation', {
      on: 'symbol',
      message: 'Periodical ID does not match',
      disabled: computed('model', function() {
        return this.model.get('isNew');
      })
    })
