...
export default Ember.Route.extend(CanMixin, {
  model() {
    let self = this;
    return this.store.findRecord('doi', this.modelFor('dois/show').get('id'), { include: 'provider,client', reload: true }).then(function(doi) {
      return doi;
  ...
  afterModel() {
    if (!this.can('delete doi', this.modelFor('dois/show'))) {
      return this.transitionTo('index');
    } else {
      this.modelFor('dois/show').set('confirmDoi', null);
    }
  }
});
