...
module('Integration | Component | doi subjects', function (hooks) {
  ...
  test('add multiple values', async function (assert) {
    ...
    let subjectSchemesInput = this.element.querySelectorAll(
      'input.subject-scheme-field'
    );
    let subjectSchemes = this.element.querySelectorAll(
      '[data-test-subject-scheme]'
    );
    assert.dom(subjectSchemesInput[0]).hasAttribute('disabled', '');
    assert
      .dom(subjectSchemes[0])
      .hasValue('Fields of Science and Technology (FOS)');

    let subjectSchemeUrisInput = this.element.querySelectorAll(
      'input.subject-scheme-uri-field'
    );
    let subjectSchemeUris = this.element.querySelectorAll(
      '[data-test-subject-uri]'
    );
    assert.dom(subjectSchemeUrisInput[0]).hasAttribute('disabled', '');
    assert
      .dom(subjectSchemeUris[0])
      .hasValue('http://www.oecd.org/science/inno/38235147.pdf');

    let subjectSchemeCodesInput = this.element.querySelectorAll(
      'input.subject-classification-code-field'
    );
    let subjectSchemeCodes = this.element.querySelectorAll(
    ...
    assert
      .dom(subjectSchemeUris[1])
      .hasValue('http://www.oecd.org/science/custom.pdf');
